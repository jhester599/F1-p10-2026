#!/usr/bin/env python3
"""
v8.19 & v8.20 — Retry top near-pass features with v8.18 (fantasy labels) baseline

With fantasy-score labels, the ranker training signal changed fundamentally.
Features that were close but failed under the old label scheme may now perform
differently due to the new gradient computation.

v8.19: circ_experience_rate = circ_races / (career_races + 1)
  Previously tested (v8.12): CV +0.46, holdout +0.12 vs 13.29 baseline.
  Retry against v8.18 baseline (13.96). Fantasy labels may amplify or dampen.

v8.20: circ_race_vs_qual = circ_avg_fin - circ_avg_qual
  Previously tested (v8.17): CV +0.58, holdout −0.50 (overfit).
  With better-calibrated ranker, overfitting may be reduced.

Baseline: v8.18 (13.96 pts/race, 51 features, fantasy-score labels)
Acceptance: delta >= +0.20 on 2025 holdout vs 13.96

Usage:
  python scripts/50_test_v819_v820_feature_retry.py
"""
import logging
import sys
...
RESULTS_DIR_V = RESULTS_DIR / "v819_v820_feature_retry"
RESULTS_DIR_V.mkdir(parents=True, exist_ok=True)

BASELINE_HOLD  = 13.96    # v8.18 verified baseline
CORR_THRESHOLD = 0.75
ACCEPT_DELTA   = 0.20
...
def test_feature(df, feat_name, train_cv, eval_cv, train_h, eval_h) -> dict:
    ...
    logger.info("Holdout: base=%.2f  +feat=%.2f  delta_vs_v818=%+.2f", h_base_ens, h_cand_ens, h_delta)
...
def main():
    logger.info("=" * 70)
    logger.info("v8.19/v8.20 RETRY: circ_experience_rate + circ_race_vs_qual")
    ...
    r19 = test_feature(df, "circ_experience_rate", train_cv, eval_cv, train_h, eval_h)
    results["v8.19_circ_experience_rate"] = r19
    logger.info("v8.19 circ_experience_rate: %s", r19["status"])

    r20 = test_feature(df, "circ_race_vs_qual", train_cv, eval_cv, train_h, eval_h)
    results["v8.20_circ_race_vs_qual"] = r20
    logger.info("v8.20 circ_race_vs_qual: %s", r20["status"])

    # Combined if both pass
    both_pass = (r19.get("status") == "ACCEPTED" and r20.get("status") == "ACCEPTED")
    if both_pass:
        logger.info("\n=== Testing COMBINED ===")
        combo_cols = list(FEATURE_COLS) + ["circ_experience_rate", "circ_race_vs_qual"]
        ...
        logger.info("Combined holdout: %.2f  delta_vs_v818=%+.2f", h_combo_ens, combo_delta)
    ...
    logger.info("\n" + "=" * 70)
    logger.info("SUMMARY (baseline: v8.18 = 13.96):")
    for name, r in results.items():
