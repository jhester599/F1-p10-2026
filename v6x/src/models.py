...
# ── v9.3 Discrete seasonal weighting gate ─────────────────────────────────────
#
...
# rf_clf relies on career/circuit history which is stable from race 1, so it
# receives a higher weight in early races.
#
# Gate design: discrete (race_num ≤ 5 → EARLY_GATE, else → normal F_soft_all).
# NOT continuous — avoids the instability seen in v3.72 3-stage adaptive weights.
#
# Early weights vs F_soft_all:
#   xgb_ranker:  6.0 → 4.0  (down-weighted: form features noisy R1-5)
#   rf_clf:      1.5 → 3.5  (up-weighted: career/circuit history stable)
#   all others:  unchanged
#
# Tested in: scripts/62_test_v93_seasonal_gate.py
ENSEMBLE_WEIGHTS_EARLY_GATE: dict[str, float] = {
    "xgb_ranker":      4.00,   # down from 6.0: form-dependent ranker, noisy R1-5
    "lgbm_ranker":     1.50,   # unchanged
    "rf_clf":          3.50,   # up from 1.5: career/circuit history stable from R1
    "xgb_clf":         0.50,   # unchanged
    "lgb_reg":         0.25,   # unchanged
    "ridge":           0.25,   # unchanged
    "rf_reg":          0.00,   # removed (same as normal)
    "xgb_reg":         0.00,   # removed (same as normal)
    "grid_heuristic":  0.00,   # removed (same as normal)
    "champ_heuristic": 0.00,   # removed (same as normal)
}

# Gate threshold: races 1–5 use EARLY_GATE weights; races 6+ use ENSEMBLE_WEIGHTS.
SEASONAL_GATE_THRESHOLD: int = 5


def get_ensemble_weights(race_num: int) -> dict[str, float]:
    """Return the appropriate ensemble weight dict for *race_num*.

    Implements a discrete seasonal gate (v9.3 Phase 5):
      - Races 1–5  (race_num ≤ SEASONAL_GATE_THRESHOLD): ENSEMBLE_WEIGHTS_EARLY_GATE
      - Races 6+   (race_num >  SEASONAL_GATE_THRESHOLD): ENSEMBLE_WEIGHTS (F_soft_all)
    ...
    -------
    dict[str, float]
        Weight dict keyed by model name.
    """
    if race_num <= SEASONAL_GATE_THRESHOLD:
        return ENSEMBLE_WEIGHTS_EARLY_GATE
