...
### League digest emails

Participant-facing spoiler digests are generated by:

```bash
python scripts/67_send_league_digest.py --year 2026 --round 9 --spreadsheet-id <sheet_id> --dry-run
```

The digest reads participant addresses from `Form Responses 1`, reads standings
and race points from the `results` tab, attaches a total-points bar chart and a
league time-series chart, and marks the subject/body with an obvious SPOILER
warning. During testing, set `RESULTS_DIGEST_TEST_RECIPIENT` to
`jeffrey.r.hester@gmail.com`; this overrides participant recipients while still
using the live league data. Scheduled workflow sends also default to that test
address unless `RESULTS_DIGEST_SEND_TO_PARTICIPANTS` is set to `true`.
...
| **Circuit** | `circ_p10_grid_chaos` | Historical-only std-dev of P10 finisher's starting position at this circuit | 5/8 models |
...
> For a narrow historical-only `circ_p10_grid_chaos` refresh, run
> `python scripts/105_refresh_circ_p10_grid_chaos.py`; the default path refreshes the combined
> parquet first and then derives the train/eval split files from that combined history.
...
- **xgb_clf tie/leakage audit (v10.x, 2026-06-15):**
  ...
  `xgb_clf` does not use that feature. The processed parquet snapshots were refreshed
  with `scripts/105_refresh_circ_p10_grid_chaos.py`, deriving split files from the
  combined 2010-2025 snapshot so prior-year circuit history is preserved.
