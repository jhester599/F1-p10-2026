#!/usr/bin/env python3
"""
v8.29 — grid_vs_season_avg feature

grid_vs_season_avg = grid_position - cumulative_season_avg_grid
(expanding mean of prior races in this season for this driver, avoiding leakage)

Captures whether driver is qualifying better/worse than their season pace:
  +5 = 5 positions worse than usual (regression candidate for race)
  -5 = 5 positions better than usual
R1: always 0 (no prior data)

max |r| with existing FEATURE_COLS = 0.524 (vs grid_position) -> ELIGIBLE

Protocol:
  1. ADD: 52 features (add grid_vs_season_avg)
  2. SWAP: replace grid_position (most correlated, |r|=0.524) with new feature

Baseline: v8.23 (14.21 pts/race)
Acceptance: delta >= +0.20 on 2025 holdout vs 14.21 (need >= 14.41)

Usage:
  python scripts/59_test_v829_grid_vs_season_avg.py
"""
import logging
import sys
import warnings
...
RESULTS_DIR_V = RESULTS_DIR / "v829_grid_vs_season_avg"
RESULTS_DIR_V.mkdir(parents=True, exist_ok=True)

BASELINE_HOLD  = 14.21
ACCEPT_DELTA   = 0.20

NEW_FEATURE  = "grid_vs_season_avg"
SWAP_FEATURE = "grid_position"   # most correlated (|r|=0.524)


def add_grid_vs_season_avg(df):
    """Compute expanding season-avg grid position delta (no future leakage)."""
    df = df.sort_values(["year", "driver_id", "round"]).copy()
    df[NEW_FEATURE] = (
        df.groupby(["year", "driver_id"])["grid_position"]
          .expanding().mean()
          .shift(1)
          .reset_index(level=[0, 1], drop=True)
    )
    df[NEW_FEATURE] = df["grid_position"] - df[NEW_FEATURE]
    df[NEW_FEATURE] = df[NEW_FEATURE].fillna(0.0)   # R1: no prior data
    return df
...
def main():
    logger.info("=" * 70)
    logger.info("v8.29 FEATURE: grid_vs_season_avg (max|r|=0.524 vs grid_position)")
    logger.info("Baseline: v8.23 (14.21 pts/race)")
    logger.info("=" * 70)

    df = pd.read_parquet(PROCESSED_DIR / "features_2010_2025.parquet")
    df = add_grid_vs_season_avg(df)
    logger.info("Dataset: %d rows  |  base FEATURE_COLS: %d  new feature added", len(df), len(FEATURE_COLS))
    logger.info("grid_vs_season_avg: mean=%.2f  std=%.2f",
    ...
    results = []
    for name, cols in [("ADD_52feat", add_cols), ("SWAP_grid_pos", swap_cols)]:
