#!/usr/bin/env python3
"""
v8.13 & v8.14 — Test: teammate_qual_delta and qual_form_trend

v8.13: teammate_qual_delta = grid_position - teammate_grid
  Intuition: Driver outqualifying teammate (negative value) signals peak form relative
  to car performance, independently from absolute qualifying gap.
  Max |r|: 0.422 with teammate_grid (below threshold).

v8.14: qual_form_trend = avg_qual_last3 - last_qual_pos
  Intuition: Qualifying momentum — analogous to drv_form_trend for finish positions.
  Negative = improving (qualifying better this race vs recent average).
  Max |r|: 0.440 with last_qual_pos (below threshold).
...
Usage:
  python scripts/46_test_v813_v814_qual_features.py
"""
import logging
import sys
...
RESULTS_DIR_V = RESULTS_DIR / "v813_v814_qual_features"
...
def add_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["qual_form_trend"]     = df["avg_qual_last3"] - df["last_qual_pos"]
    return df
...
def main():
    logger.info("=" * 70)
    logger.info("v8.13/v8.14 TEST: teammate_qual_delta + qual_form_trend")
    logger.info("=" * 70)
    ...
    logger.info("teammate_qual_delta: mean=%.3f  std=%.3f",
                df["teammate_qual_delta"].mean(), df["teammate_qual_delta"].std())
    logger.info("qual_form_trend: mean=%.3f  std=%.3f",
                df["qual_form_trend"].mean(), df["qual_form_trend"].std())
    ...
    # ── Test v8.13: teammate_qual_delta ──────────────────────────────────────
    r13 = test_feature(df, "teammate_qual_delta", train_cv, eval_cv, train_h, eval_h)
    results["v8.13_teammate_qual_delta"] = r13
    logger.info("v8.13 teammate_qual_delta: %s", r13["status"])

    # ── Test v8.14: qual_form_trend ──────────────────────────────────────────
    r14 = test_feature(df, "qual_form_trend", train_cv, eval_cv, train_h, eval_h)
    results["v8.14_qual_form_trend"] = r14
    logger.info("v8.14 qual_form_trend: %s", r14["status"])

    # ── If both pass, test combined ───────────────────────────────────────────
    both_pass = (r13.get("status") == "ACCEPTED" and r14.get("status") == "ACCEPTED")
    if both_pass:
        logger.info("\n=== Testing COMBINED addition ===")
        combo_cols = list(FEATURE_COLS) + ["teammate_qual_delta", "qual_form_trend"]
