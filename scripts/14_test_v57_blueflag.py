#!/usr/bin/env python3
"""
14_test_v57_blueflag.py — v5.7 Blue Flag Vulnerability Feature Evaluation

Tests blue_flag_vulnerability against the v5.6 baseline (48-feature set).
Feature is added individually to FEATURE_COLS; delta measured on 2024 single-fold CV.

Protocol (same as v5.2, v5.5, v5.6):
...
Results saved to scripts/v5_results/v57_blueflag_results.csv

Usage:
  python scripts/14_test_v57_blueflag.py
"""
...
CANDIDATES = [
    (
        "blue_flag_vulnerability",
        "q_gap_pct / 100 × race_laps: expected laps deficit vs leader at race end. "
        "≥1.0 = driver likely to be lapped, causing 1.5–3s time losses. "
        "Captures pace-gap × circuit-length interaction.",
    ),
]
...
def main() -> None:
    ...
    for feat, _ in CANDIDATES:
        ...
        n_ge1 = (vals >= 1.0).sum()
        logger.info(
            "  Train %s: mean=%.3f  std=%.3f  >0: %d (%.0f%%)  ≥1.0 (lapping risk): %d (%.0f%%)",
            feat, vals.mean(), vals.std(),
            n_pos, n_pos / len(tr_df) * 100,
            n_ge1, n_ge1 / len(tr_df) * 100,
        )
    ...
    logger.info("\n=== BASELINE (v5.6, %d features) ===", len(FEATURE_COLS))
    ...
    logger.info("\n\n=== v5.7 Blue Flag Vulnerability Feature Evaluation Summary ===")
    logger.info("Baseline (v5.6, %d features): rf_reg=%.3f  lgb_reg=%.3f  rf_clf=%.3f  xgb_clf=%.3f",
                len(FEATURE_COLS),
    ...
    out_path = RESULTS_DIR / "v57_blueflag_results.csv"
